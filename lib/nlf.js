...
function parseInstalled(data, options, callback) {
	...
	(function parseModuleData(moduleData, currentDepth, output) {
		...
		if (options.production && (isDevDependency(moduleData) || moduleData.extraneous)) {
			return;
		}
...
function createModule(moduleData, callback) {
	...
	var repository = (moduleData.repository || {}).url ||
	    moduleData.repository || moduleData.homepage || '(none)';
  ...
  var module = new Module(id, name, version, directory, repository, moduleData.depType);
...
function find(options, callback) {
	...
	var depOptions = {
		dev: true,
		extraFields: [ 'repository', 'licenses', 'license', 'homepage', 'author' ]
	};
